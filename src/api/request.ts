...
let baseUrl: string = localStorage.getItem('app_server_url') || ''
...
export function setBaseUrl(url: string): void {
  ...
  if (trimmed && trimmed !== DEFAULT_SERVER_URL) {
    localStorage.setItem('app_server_url', trimmed)
  } else {
    localStorage.removeItem('app_server_url')
  }
}
...
async function request<T>(method: string, path: string, body?: unknown): Promise<ApiResponse<T>> {
  ...
  try {
    ...
    if (res.status === 401) { handleUnauth(); return { code: 'E_UNAUTH', info: '登录已过期', data: null } }
  ...
  fetch(url, { method: 'POST', headers, body: JSON.stringify(body), signal: controller.signal })
    .then(async (res) => {
      ...
      while (true) {
        ...
        if (text) {
          if (text.includes('[TIMEOUT]')) { onError('响应超时，请稍后重试'); reader.cancel(); return }
          onChunk(text)
        }
      }
      onDone()
